...
class Distributor_provider_contact_model extends CI_Model
{
    ...
    public function getListingData($config_arr = array())
    {
        ...
        */
        if(isset($config_arr['iProviderId'])&& $config_arr['iProviderId']!='')
          $this->db->where("dpc.iProviderId",$config_arr['iProviderId'], FALSE, FALSE);
        if(isset($config_arr['iUserId'])&& $config_arr['iUserId']!='')  
          $this->db->where("dpc.iUsersId",$config_arr['iUserId'],false,false);
